    ...
    <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto text-center px-4 py-24 pt-32">
        ...
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-serif font-medium text-white mb-6 leading-tight">
          {title}
          {subtitle && (
            <span className="block text-[#7A6208] mt-2">{subtitle}</span>
          )}
        </h1>
        ...
        {price && (
          <p className="text-lg text-[#7A6208] font-medium mb-8">
            {price}
          </p>
        )}
...
export function StatCard({ value, label, color = 'gold' }: StatCardProps) {
  const colorClasses = {
    gold: 'border-[#7A6208]/20 text-[#7A6208]',
...
export function FeatureCard({ title, description, note }: FeatureCardProps) {
  return (
    <div className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
      ...
      {note && <p className="text-[#7A6208] text-xs font-medium">{note}</p>}
    </div>
  )
}
  ...
  return (
    <div className="text-center">
      <div className="w-14 h-14 mx-auto mb-4 rounded-full bg-white border-2 border-[#7A6208] flex items-center justify-center shadow-sm">
        <span className="text-[#7A6208] font-serif text-lg">{number}</span>
      </div>
  ...
  return (
    <section className="py-16 md:py-24 px-4 bg-[#F7F5F3]">
      <div className="max-w-4xl mx-auto text-center">
        ...
        <a
          href={buttonHref}
          className="inline-block px-10 py-4 bg-[#7A6208] hover:bg-[#5C4B06] text-white font-medium rounded-full transition-colors uppercase tracking-wider text-sm"
        >
          {buttonText}
        </a>
      </div>
