        ...
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3" aria-label="Membership Marketing Agency - Home">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
              scrolled ? 'bg-[#B8860B]' : 'bg-white/20 backdrop-blur-sm'
            }`}>
          ...
          <nav className="hidden md:flex items-center gap-1">
                ...
                <div className="absolute top-full left-0 mt-2 w-56 bg-white border border-gray-100 rounded-xl shadow-xl py-2 z-50">
                  {services.map((service) => (
                    <Link
                      key={service.href}
                      href={service.href}
                      className="block px-4 py-3 text-sm text-gray-700 hover:text-[#B8860B] hover:bg-gray-50 transition-colors"
                    >
                      {service.label}
                    </Link>
                  ))}
            ...
            {isPending ? (
              ...
              <Link
                ...
                className="bg-[#B8860B] hover:bg-[#996F0A] text-white px-6 py-2.5 rounded-full text-sm font-medium tracking-wide transition-colors"
              >
                Book Consultation
              </Link>
            )}
            ...
            <div className="flex flex-col">
              ...
              {services.map((service) => (
                <Link
                  ...
                  className="px-6 py-3 text-gray-700 hover:text-[#B8860B] hover:bg-gray-50 transition-colors"
                >
                  {service.label}
                </Link>
              ))}
              ...
              <div className="pt-4 px-6 border-t border-gray-100 mt-4">
                {!isPending && !user && (
                  <Link
                    href="#book-consultation"
                    onClick={() => setMobileMenuOpen(false)}
                    className="block bg-[#B8860B] hover:bg-[#996F0A] text-white px-6 py-3 rounded-full text-center font-medium transition-colors"
                  >
                    Book Consultation
                  </Link>
                )}
